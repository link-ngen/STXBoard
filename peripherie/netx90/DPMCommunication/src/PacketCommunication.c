...
typedef __HIL_PACKED_PRE struct PKT_INTERFACE_HANDLER_Ttag
{
  Pkt_HandlePacketCallback pfnHandler;
  void *pvUserData;
} __HIL_PACKED_POST PKT_INTERFACE_HANDLER_T;

/** Packet communication interface resources */
typedef __HIL_PACKED_PRE struct PKT_INTERFACE_Ttag
{
...
} __HIL_PACKED_POST PKT_INTERFACE_T;
 ...
 *
 */
static void Pkt_PrintHexData(unsigned char* pbData, unsigned long  ulDataLen )
{
 ...
 *
 */
static void Pkt_PrintPacket( CIFX_PACKET* ptPacket )
{
 ...
 *
 */
static uint32_t Pkt_PutPacket(CIFXHANDLE hCifXChannel, CIFX_PACKET *ptPacket, uint32_t ulTimeout)
{
  ...
  {
    PRINTF("Packet sent:" NEWLINE); Pkt_PrintPacket(ptPacket);
  }
  else
  {
    PRINTF("Packet sent, failed with error: 0x%08x" NEWLINE, (unsigned int) lRet); Pkt_PrintPacket(ptPacket);
  }
 ...
 *
 */
static uint32_t Pkt_GetPacket(CIFXHANDLE hCifXChannel, CIFX_PACKET *ptPacket, uint32_t ulTimeout)
{
  uint32_t lRet = CIFX_NO_ERROR;
  ...
  if(CIFX_NO_ERROR == lRet)
  {
    PRINTF("Packet received:" NEWLINE); Pkt_PrintPacket(ptPacket);
  }
 ...
 *
 */
static void Pkt_DispatchIndication(PKT_INTERFACE_T *ptPktIfRsc, CIFX_PACKET *ptPacket)
{
  int   i           = 0;
...
{
  PKT_INTERFACE_T *ptPktIfRsc = OS_Memcalloc(1, sizeof(PKT_INTERFACE_T));
...
/**************************************************************************************************/
/**************************************************************************************************/
bool Pkt_RegisterPacketHandler(PKT_INTERFACE_T *ptPktIfRsc, Pkt_HandlePacketCallback pfnHandler, void *pvUserData)
{
  int i = 0;
...
/**************************************************************************************************/
void Pkt_CheckReceiveMailbox(PKT_INTERFACE_T *ptPktIfRsc, CIFX_PACKET *ptPacket)
{
...
/**************************************************************************************************/
uint32_t Pkt_SendPacket(PKT_INTERFACE_T *ptPktIfRsc, CIFX_PACKET *ptPacket, uint32_t ulTimeout)
{
  uint32_t ulRet = CIFX_NO_ERROR;
...
/**************************************************************************************************/
uint32_t Pkt_SendReceivePacket(PKT_INTERFACE_T *ptPktIfRsc, CIFX_PACKET *ptPacket, uint32_t ulTimeout)
{
  uint32_t ulRet = CIFX_NO_ERROR;
