...
/*! Assembles a HIL_REGISTER_REQ packet.
 *
...
/*! Assembles a HIL_UNREGISTER_REQ packet.
 *
 * \param ptPkt   [out] Packet resource used for assembling.
 */
void SysPkt_AssembleUnRegisterAppReq(CIFX_PACKET *ptPkt)
{
  HIL_UNREGISTER_APP_REQ_T *ptReq = (HIL_UNREGISTER_APP_REQ_T*) ptPkt;

  memset(ptReq, 0, sizeof(*ptReq));

  ptReq->tHead.ulDest = HIL_PACKET_DEST_DEFAULT_CHANNEL;
  ptReq->tHead.ulCmd = HIL_UNREGISTER_APP_REQ;
  ptReq->tHead.ulLen = 0;
}

/*! Evaluates a HIL_REGISTER_REQ packet
 *
 *  \param  ptPkt [in] Pointer to confirmation Packet
 */
void SysPkt_HandleRegisterAppCnf(CIFX_PACKET *ptPkt)
{
}
 ...
 *  \param  ptPkt [in] Pointer to confirmation Packet
 */
void SysPkt_HandleChannelInitCnf(CIFX_PACKET *ptPkt)
{
}
 ...
 *  \param  ptPkt [in] Pointer to confirmation Packet
 */
void SysPkt_HandleStartStopCommCnf(CIFX_PACKET *ptPkt)
{
}
 ...
 *  \param  ptPkt [in] Pointer to confirmation Packet
 */
void SysPkt_HandleSetMacAddressCnf(CIFX_PACKET *ptPkt)
{
}
 ...
 *  \param  ptPkt [in] Pointer to confirmation Packet
 */
void SysPkt_HandleFirmwareResetCnf(CIFX_PACKET *ptPkt)
{
}
 ...
 *  \param  ptPkt [in] Pointer to confirmation Packet
 */
void SysPkt_HandleHardwareInfoCnf(CIFX_PACKET *ptPkt)
{
...
void SysPkt_PD0InTriggeredReq(CIFX_PACKET *ptPkt)
{
  ...
  tSetTriggerReq->tHead.ulLen = sizeof(tSetTriggerReq->tData);
  tSetTriggerReq->tHead.ulDest = HIL_PACKET_DEST_SYSTEM;
  tSetTriggerReq->tData.usPdInHskTriggerType = HIL_TRIGGER_TYPE_PDIN_RX_DATA_RECEIVED;
  tSetTriggerReq->tData.usPdOutHskTriggerType = HIL_TRIGGER_TYPE_PDOUT_READY_FOR_TX_DATA;
